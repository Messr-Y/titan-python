...
CLASSES = ['background', 'glove', 'noglove']
root = 'C:/Users/HP/pythonProject/VOC2022/'
...
class VOCDataset(torch.utils.data.Dataset):
    def __init__(self, img_ids, transforms=None):
        global num_noglove, num_glove, flag
        ...
        for image_file in img_ids:
            ...
            for obj in annotation_file.findall("object"):
                ...
                if label == 'noglove' and flag:
                    num_noglove += 1
                elif label == 'glove' and flag:
                    num_glove += 1
            ...
            # print("{}:{}".format(image_file, a.size()))
...
batch_size = 4 # 一般设为32以上，我的电脑只能支持到2
num_epochs = 50 # 训练次数
lr = 1e-4 # 学习率
...
# checkpoint = torch.load('checkCoat.pth')
...
print("num_noglove={}".format(num_noglove))
print("num_glove={}".format(num_glove))
...
def test(model, loader, device, score_threshold=0.7, iou_threshold=0.1,max_num_targets=100):
    ...
    print("绝缘手套个数:{}".format(hat))
    print("手的个数:{}".format(person))

    # 返回预测结果，准确率，召回率
    return results, tp/select_boxes, tp/all_true_boxes
